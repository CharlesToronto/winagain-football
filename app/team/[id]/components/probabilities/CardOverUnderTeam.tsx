...
type Fixture = any;
export type GoalFocus = "for" | "against";
...
function computeOverUnder(fixtures: Fixture[] = [], mode: Mode, focus: GoalFocus) {
...
export default function CardOverUnderTeam({
  fixtures,
  opponentFixtures,
  ...
  const stats = useMemo(
    () => computeOverUnder(fixtures, mode, goalFocus),
    [fixtures, mode, goalFocus]
  );
  const opponentStats = useMemo(
    () => computeOverUnder(opponentFixtures ?? [], mode, goalFocus),
    [opponentFixtures, mode, goalFocus]
  );
  ...
  return (
    <div className="bg-sky-400/10 backdrop-blur-sm rounded-xl p-6 shadow h-[20rem]">
