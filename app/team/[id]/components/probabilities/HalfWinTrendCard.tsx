...
function buildBarsFromMatches(matches: MatchEntry[]) {
  ...
  const maxOffset = Math.max(0, (slotWidth - barWidth) * 0.5);
  const halfOffset = Math.min(
    Math.max(slotWidth * 0.12, barWidth * 0.6),
    maxOffset
  );
  ...
  matches.forEach((match, idx) => {
    ...
    const useOffset = hasWin && hasLoss;
    const winX = useOffset ? centerX - halfOffset : centerX;
    const lossX = useOffset ? centerX + halfOffset : centerX;
